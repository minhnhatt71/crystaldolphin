...
func (q *QQChannel) Name() channelmodel.ChannelName { return channelmodel.ChannelQQ }
...
func (q *QQChannel) getAccessToken(ctx context.Context) (string, error) {
	...
	q.tokenExp = time.Now().Add(7100 * time.Second)
	return q.token, nil
}
...
func (q *QQChannel) Send(ctx context.Context, msg channelmodel.Message) error {
	token, err := q.getAccessToken(ctx)
	if err != nil {
		return err
	}
