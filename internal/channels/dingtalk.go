...
func NewDingTalkChannel(cfg *channel.DingTalkConfig, b *bus.InboundBus) *DingTalkChannel {
	return &DingTalkChannel{
		Base:       NewBase(channelmodel.ChannelDingTalk, b, cfg.AllowFrom),
		cfg:        cfg,
		httpClient: &http.Client{Timeout: 15 * time.Second},
	}
}

func (d *DingTalkChannel) Name() channelmodel.ChannelName { return channelmodel.ChannelDingTalk }
...
func (d *DingTalkChannel) Send(ctx context.Context, msg channelmodel.Message) error {
	token, err := d.getAccessToken(ctx)
	if err != nil {
		return err
	}
