...
func NewFeishuChannel(cfg *channel.FeishuConfig, b *bus.InboundBus) *FeishuChannel {
	return &FeishuChannel{
		Base:       NewBase(channelmodel.ChannelFeishu, b, cfg.AllowFrom),
		cfg:        cfg,
		httpClient: &http.Client{Timeout: 15 * time.Second},
	}
}

func (f *FeishuChannel) Name() channelmodel.ChannelName { return channelmodel.ChannelFeishu }
...
func (f *FeishuChannel) Send(ctx context.Context, msg channelmodel.Message) error {
	token, err := f.getAccessToken(ctx)
	if err != nil {
		return err
	}
