...
func (s *SlackChannel) Name() channelmodel.ChannelName { return channelmodel.ChannelSlack }
...
func (s *SlackChannel) handleInnerEvent(ev slackevents.EventsAPIInnerEvent) {
	...
	if !s.isAllowedSlack(userID, ch, channelType) {
		return
	}
	if channelType != "im" && !s.shouldRespond(ev.Type, text, ch) {
		return
	}
	...
	s.HandleMessage(userID, ch, text, nil, map[string]any{
...
func (s *SlackChannel) isAllowedSlack(user, ch, channelType string) bool {
	if channelType == "im" {
		if !s.cfg.DM.Enabled {
			return false
		}
...
func (s *SlackChannel) shouldRespond(evType, text, ch string) bool {
...
func (s *SlackChannel) Send(ctx context.Context, msg channelmodel.Message) error {
	if s.webClient == nil {
		return nil
	}
	slack := map[string]any{}
