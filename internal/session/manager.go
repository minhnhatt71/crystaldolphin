...
func (m *Manager) GetOrCreate(key string) *ChannelSessionImpl {
	if v, ok := m.cache.Load(key); ok {
		return v.(*ChannelSessionImpl)
	}
	...
	return actual.(*ChannelSessionImpl)
}
...
func (m *Manager) load(key string) schema.ChannelSession {
